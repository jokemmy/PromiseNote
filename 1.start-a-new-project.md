...
# shim
$ npm i es5-shim es6-shim console-polyfill
```


复制 `es5-shim.min.js` `es5-sham.min.js` `es6-shim.min.js` `es6-sham.min.js` `console-polyfill/index.js` 文件到 `public` 文件夹
...
> 由于 `roadhog` 中 `webpack` 配置的依赖包 `loader-utils` 有一些 **bug**，所以会有字体本地化的不成功的问题！需要等待版本更新，问题才能得以解决。

临时的解决方案：手动修改 `loader-utils/index.js` 文件，拷贝以下代码覆盖 `stringifyRequest` 函数

```js
// 写于2017-2-8 版本`loader-utils@0.2.16` 如果问题已经修复，请忽略这段代码 
...
除了 `react/sort-comp` 其他可以按照自己的书写习惯适当修改
...
1. `public/index.html`
  ...
  ```html
    ...
    <head>
      ...
      <script type="text/javascript" src="es6-sham.min.js"></script>
